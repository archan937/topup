class Comment < ActiveRecord::Base
  ...
  def spam?
    self.message.match(/(^Very good site\. Thanks\!\,|^Nice site\. Thank you\.|^hi <a[^>]+>[^<\/]+<\/a>\s*$|http\:\/\/(ui\-patterns\.com|tezzels\.net)\/users\/\d+\#\d|freehostia\.com|\bporn\b|\bsex\b)/) or self.message.count_matches(/<a[^>]+>[^<\/]+<\/a>/) > 8
  end
