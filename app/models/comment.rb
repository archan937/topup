class Comment < ActiveRecord::Base
  ...
  def spam?
    self.message.downcase.match(/(^very good site\. thanks\!\,|^nice site\. thank you\.|^hi <a[^>]+>[^<\/]+<\/a>\s*$|http\:\/\/(ui\-patterns\.com|tezzels\.net)\/users\/\d+\#\d|freehostia\.com|^comment\d+,|\b(porn|sex|nude|xxx|fuck|playboy)\b|\[\/url\]|^\d\s<a([^>]+)?>[^<\/]+<\/a>\s+$)/) ||
    self.message.count_matches(/<a([^>]+)?>[^<\/]+<\/a>/) > 8
  end
